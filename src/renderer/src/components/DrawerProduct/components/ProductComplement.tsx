...
export const ProductComplement: React.FC = () => {
	...
	function addComplementItem(complementId: string) {
		...
		if (complementIndex !== -1) {
			...
			if (!updatedComplements[complementIndex].complement_items) {
				...
				updatedComplements[complementIndex].complement_items = [
					newComplementItem,
				];
			} else {
				// Caso contrário, adicione o novo complement_item ao array existente
				updatedComplements[complementIndex].complement_items?.push(
					newComplementItem
				);
			}
				...
				{complements.length > 0 ? (
					complements.map((complement) => (
						<ComplementForm
							...
							OnChangeAddItem={(complementId) =>
								addComplementItem(complementId)
							}
						/>
					))
				) : (
