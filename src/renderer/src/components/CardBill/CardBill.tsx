...
export const CardBill: React.FC<CardBillProps> = (props: CardBillProps) => {
  ...
  return (
    <S.Container onClick={(): void => navigate(`/comandas/${props.data.id}`)}>
      <S.InfoBill>
        <div>
          <Avatar
            size={48}
        ...
        {props.data.client_name && (
          <Text strong style={{ fontSize: '18px' }}>
            ...
            {props.data.client_name.split(' ').length > 2
              ? `${props.data.client_name.split(' ')[0]} ${props.data.client_name.split(' ')[1]}`
              : `${props.data.client_name}`}
          </Text>
        )}
      ...
      {/* <S.FinanceBill>
        ...
        <Divider type="vertical" style={{ height: '100%', margin: '0' }} orientation="center" />
        <S.BoxFinance>
