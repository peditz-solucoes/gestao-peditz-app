...
export const BillComponent: React.FC = () => {
  ...
  const removeCaracteres = (value: string): string => {
    return value.replace(/[^Mm0-9]/g, '')
  }

  const filterBills = (): void => {
      ...
      <S.container>
        ...
        >
          <Input.Search
            ...
            value={searchValue}
            onChange={(e): void => setSearchValue(removeCaracteres(e.target.value))}
            style={{
              width: '300px'
            }}
            ...
            size="large"
          />
          <Button type="primary" size="large" onClick={(): void => setShowModal(true)}>
            Abrir Comanda
          </Button>
