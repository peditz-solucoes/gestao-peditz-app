...
export const Command: React.FC = () => {
	const [visibleJoinCommandModal, setVisibleJoinCommandModal] =
	...
	const { fetchBill, selectedBill, handleDeleteOrder, ordersGroupList } =
		useBill();
	const { id } = useParams();
		...
		confirm({
			...
			content: (
				...
				>
					...
					<Text>
						Preço do item:{' '}
						<strong>{formatCurrency(Number(props.price))}</strong>
					</Text>
					<Text>
						...
						>
							...
							>
								...
								>
									{formatCurrency(120)}
									<Badge
										count={<BsFillDatabaseFill style={{ color: '#a49d16' }} />}
									/>
								</Text>
							</div>
						</div>
